<section class="rounded-lg border border-slate-200 bg-white shadow-sm">
    <div class="border-b border-slate-200 px-5 py-4">
        <h2 class="text-xl font-semibold text-slate-900">Netzwerkmap</h2>
        <p class="mt-1 text-sm text-slate-500">
            Die Karte basiert auf allen gespeicherten Profilen, Personen und Beziehungen im System. Standardmaessig wird der Verbindungsfilter so gesetzt, dass hoechstens 100 Profile sichtbar sind.
        </p>
    </div>

    @if (! $tablesAvailable)
        <div class="p-5 text-sm text-amber-900">
            Fuer die Netzwerkmap werden `instagram_profiles` und `instagram_profile_relationships` benoetigt.
        </div>
    @else
        <div
            class="bg-[#fafafa]"
            ...
            data-network-filter-scope="admin-global"
                ...
                <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                    ...
                    <div class="grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
                        ...
                        <div class="rounded-lg border border-slate-200 bg-white px-4 py-3 shadow-sm">
                            ...
                            <div class="mt-1 text-xl font-bold text-slate-950">{{ number_format($stats['profiles']) }}</div>
                        </div>
                        <div class="rounded-lg border border-slate-200 bg-white px-4 py-3 shadow-sm">
                    ...
                    <section class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
                        ...
                        @if(empty($graph['nodes']))
                            <div class="p-8 text-sm text-slate-500">
                                Noch keine Netzwerkdaten vorhanden.
                            </div>
                        @else
                                ...
                                <div data-network-loading-panel class="absolute left-4 top-4 z-10 w-[min(420px,calc(100%-2rem))] rounded-lg border border-slate-200 bg-white/95 p-4 shadow-sm backdrop-blur">
                                    <div class="flex items-start justify-between gap-3">
                                        <div>
                                            ...
                                            <div class="mt-1 text-xs leading-5 text-slate-500" data-network-build-text>Die gespeicherten Profile werden dargestellt.</div>
                                        </div>
                    ...
                    <aside class="space-y-4">
                        ...
                        <div class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
                            ...
                            <div class="mt-3 space-y-2 text-sm text-slate-600">
                                <div class="flex items-center gap-2"><span class="h-2 w-8 rounded-full bg-emerald-600"></span> Follower-, Gefolgt- und Profil-Relationen</div>
                                <div class="flex items-center gap-2"><span class="h-4 w-4 rounded-full bg-amber-400 ring-2 ring-amber-500"></span> Primaere Person</div>
                                <div class="flex items-center gap-2"><span class="h-4 w-4 rounded-full bg-slate-950"></span> Beobachtete Person</div>
                                <div class="flex items-center gap-2"><span class="h-4 w-4 rounded-full bg-slate-100 ring-1 ring-slate-300"></span> Gespeichertes Instagram-Profil</div>
                            </div>
                        </div>
                    </aside>
                </div>
            ...
            <div class="flex max-h-[calc(100vh-2rem)] flex-col overflow-hidden sm:max-h-[85vh]">
                <div class="flex items-start justify-between gap-3 border-b border-slate-200 px-4 py-3 sm:px-5 sm:py-4">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">Profilvorschau aus der globalen Admin-Netzwerkmap.</p>
                    </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid gap-2 sm:grid-cols-3">
                                ...
                                <div class="rounded-lg border border-slate-200 bg-white p-3">
                                    ...
                                    <div class="mt-1 text-lg font-bold text-slate-950">{{ number_format($profilePreview['incoming_count'] ?? 0) }}</div>
                                </div>
                            </div>
                                ...
                                <div class="mt-3 space-y-2">
                                    @forelse($profilePreview['list_scans'] ?? [] as $scan)
                                        <div class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
                                            ...
                                            @if($scan['status_message'] ?? null)
                                                <div class="mt-1">{{ $scan['status_message'] }}</div>
                                            @endif
                                        </div>
                                    @empty
                                        <p class="text-sm text-slate-500">Noch keine Scan-Historie gespeichert.</p>
                                    @endforelse
                                </div>
                            </div>

                            <div class="rounded-lg border border-slate-200 bg-white p-3">
                                <div class="text-sm font-bold text-slate-950">Verknuepfte Personen</div>
                                <div class="mt-3 flex flex-wrap gap-2">
                                    @forelse($profilePreview['linked_people'] ?? [] as $linkedPerson)
                                        @if($linkedPerson['detail_url'])
                                            <a href="{{ $linkedPerson['detail_url'] }}" wire:navigate class="rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-800 hover:bg-blue-100">
                                                {{ $linkedPerson['display_name'] }}@if($linkedPerson['user_name']) · {{ $linkedPerson['user_name'] }}@endif
                                            </a>
                                        @else
                                        ...
                                        @endif
                                    @empty
                                        <p class="text-sm text-slate-500">Keine Person ist aktuell mit diesem Profil verknuepft.</p>
                                    @endforelse
                                </div>
                            </div>
                        </div>
