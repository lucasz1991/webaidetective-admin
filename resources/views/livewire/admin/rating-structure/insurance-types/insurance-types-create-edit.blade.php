            ...
            <div class="space-y-4">
                ...
                <div x-data="{ openTab: 'incurance' }" class="">
                    ...
                    <div class="flex -mb-[1px] space-x-2">
                        <button @click="openTab = 'incurance'" 
                            :class="openTab === 'incurance' ? 'text-blue-600 border-blue-600 bg-gray-100  border-b-0' : 'text-gray-500 bg-white'" 
                            ...
                            <h1 class="flex items-center justify-center">
                                ...
                                <span class="ml-2 bg-white text-sky-600 text-xs shadow border border-sky-200 font-bold aspect-square px-2 py-1 flex items-center justify-center rounded-full h-7 leading-none">
                                    {{ count($assignedInsurances) }}
                                </span>
                            </h1>
                        </button>
                        <button @click="openTab = 'incuranceSubType'" 
                            :class="openTab === 'incuranceSubType' ? 'text-blue-600 border-blue-600 bg-gray-100 border-b-0' : 'text-gray-500 bg-white'" 
                            class="px-4 py-2  text-sm font-medium transition-all border  border-gray-300 rounded-t-lg z-30">
                            <h1 class="flex items-center justify-center">
                                <span class="w-max">Versicherungsarten</span>
                                <span class="ml-2 bg-white text-sky-600 text-xs shadow border border-sky-200 font-bold aspect-square px-2 py-1 flex items-center justify-center rounded-full h-7 leading-none">
                                    {{ count($assignedInsuranceSubTypes) }}
                                </span>
                            </h1>
                        </button>
                    </div>
                                ...
                                <div class="mt-4" x-data="{ addInsuranceOpen: false }">
                                    ...
                                    <div x-show="addInsuranceOpen" @click.away="addInsuranceOpen = false" class="mt-2 mb-5  bg-white border rounded">
                                                <div class="p-2 flex items-center space-x-3">
                                                    ...
                                                    <button  wire:click="addInsurance" class="flex items-center text-sm px-2 py-1 bg-white border rounded shadow-sm hover:bg-gray-50">
                                                        <svg class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"></path>
                                                        </svg>
                                    ...
                                    <div class="min-w-max lg:min-w-full max-h-[30vh] overflow-y-scroll p-3 bg-white scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 shadow-inner border scroll-container" x-sort="$dispatch('reorderAssignedInsurances', { item: $item, position: $position })">
                                        @foreach ($assignedInsurances as $insurance)
                                            <div x-sort:item="{ id: {{ $insurance['id'] }}, name: '{{ $insurance['name'] }}' }">
                                                <div class="bg-blue-50 px-3 py-2 rounded flex justify-between items-center border mb-2">
                                                    ...
                                                    <button type="button" class="text-red-500" wire:click="removeInsurance({{ $insurance['id'] }})">
